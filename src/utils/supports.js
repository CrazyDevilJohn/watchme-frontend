...
export const fetchFeedsQuery = `*[_type == "pin"]{
 image{
      asset->{
        url
      }
...
export const fetchFeedsByFilterQuery = (category) => {
...
export const feetchFeedsByCategoryQuery = (feed) => {
  const query = `*[_type == "pin" && category == "${feed?.category}" && _id != "${feed?._id}"]{
    title,
 image{
      asset->{
        url
...
export const fetchFeedBysearchQuery = (string) => {
  const query = `*[_type == "pin" && (${
    string === "*"
      ? true
      : ` title match "${string}*" || category match "${string}*"`
  })]{
    title,
 image{
      asset->{
